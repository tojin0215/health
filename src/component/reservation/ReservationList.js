...
const ReservationList = ({ reservation, reservationDelete }) => {
	const columns = [
		...
		// },
		{
			name: '인원수',
			selector: (row) =>
				row.number_of_peopleFountain == 0
					? '제한없음'
					: row.number_of_peopleFountain + '/' + row.number_of_people,
		},
		{
			name: '삭제',
			selector: (row) => (
				<button className='deleteButton' onClick={() => reservationDelete(row)}>
					삭제
				</button>
			),
		},
	...
	return <DataTable columns={columns} data={reservation} />;
};
