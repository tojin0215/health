...
class AssignCustomer extends Component {
  ...
  cusFetch = () => {
    if (this.state.member_no == '0') {
      ...
      fetch(
        ip +
          '/assignexercise?type=customer&startDate=' +
          startTime +
          '&endDate=' +
          endTime +
          '&member_no=' +
          ...
          if (res.length == 0) {
            ...
            for (let i = res.length - 1; i >= 0; i--) {
              arr1.push({
                ...
                exercise_no: res[i].exercise_no,
                name: res[i].name,
                createAt: moment(res[i].createAt).format('YYYY/MM/DD'),
                part: res[i].part,
                machine: res[i].machine,
                data: res[i].data,
                rest_second: res[i].rest_second,
                set_count: res[i].set_count,
              });
            }
...
export default connect(
  AssignCustomerStateToProps,
  AssignCustomerDispatchToProps
)(AssignCustomer);
