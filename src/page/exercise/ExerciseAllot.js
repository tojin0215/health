...
const AssignexerciseView = ({ member_no, name, group_no }) => {
  return (
    <div>
      {member_no}, {name}, {group_no}
    </div>
  );
};

const ExerciseView = ({ part, name, machine }) => {
  const region =
    part === 1
      ? '상체'
      : part === 18
      ...
      <div>
        ...
        <div className='container'>
          ...
          <div>
            ...
            {this.state.exerciseAllotlist ? this.state.exerciseAllotlist : ''}
          </div>
        </div>
      </div>
    );
