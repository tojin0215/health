...
class AddExercise extends Component {
    ...
    handleChange = (e) => {
        ...
        if (e.target.name === "part") {
            if (e.target.checked) {
                this.setState({[e.target.name]: JSON.parse(JSON.stringify({data: Number(this.state.part)}))['data'] + Number(e.target.value)});
            } else {
                this.setState({[e.target.name]: JSON.parse(JSON.stringify({data: Number(this.state.part)}))['data'] - Number(e.target.value)});
            }
        } else if (e.target.name==="default_set_count") {
            if (/^\d+$/.test(e.target.value)) {this.setState({[e.target.name]: Number(e.target.value)});}
            else {
                if (e.target.value==="") {this.setState({[e.target.name]: ""}); return;}
    ...
    search = () =>{
        ...
        if (it==='2') {
            let v = 0;
            if (/상체/.test(this.state.search)){search = search.replace("상체", ""); v = v + 1}
            if (/하체/.test(this.state.search)){search = search.replace("하체", ""); v = v + 2}
            if (/전신/.test(this.state.search)){search = search.replace("전신", ""); v = v + 4}
            if (/코어/.test(this.state.search)){search = search.replace("코어", ""); v = v + 8}
            if (/유산소/.test(this.state.search)){search = search.replace("유산소", ""); v = v + 16}
    ...
    AddExercise = () => {
        ...
        if (this.state.name==="") {alert("이름이 없습니다"); return;}
        else if (this.state.part===0) {alert("운동 부위가 선택되지 않았습니다"); return;}
        else if (this.state.machine==="") {alert("운동 기구가 없습니다"); return;}
        else if (this.state.url==="") this.setState({url: ""});
        else if (this.state.default_data_type==="") this.setState({default_data_type: 1});
        else if (this.state.default_data==="") {alert("운동 기본 횟수가 없습니다"); return;}
        else if (this.state.default_rest_second==="") {alert("쉬는 시간이 없습니다"); return;}
        else if (this.state.default_set_count==="") {alert("운동 세트 횟수가 없습니다"); return;}
        ...
        return (
            ...
            <div className='container'>
                ...
                <div className="input-row">
                                ...
                                <input id="name" placeholder="name" name="name" onChange={this.handleChange} />
                            </div>
                            
                            <div className="input-row">
                                <label className="label-description">운동 기구</label>
                                <input id="tool" placeholder="machine" name="machine" onChange={this.handleChange} />
                            </div>
                            
                            <div className="input-row">
                                <label className="label-description">부위</label>
                                <div className="part">
                                   <label><input type="checkBox" value='1' name="part" onChange={this.handleChange}/>상체</label>
                                   <label><input type="checkBox" value='2' name="part" onChange={this.handleChange}/>하체</label>
                                   <label><input type="checkBox" value='4' name="part" onChange={this.handleChange}/>전신</label>
                                   <label><input type="checkBox" value='8' name="part" onChange={this.handleChange}/>코어</label>
                                   <label><input type="checkBox" value='16' name="part" onChange={this.handleChange}/>유산소</label>
                                </div>
                            </div>

                            <div className="input-row">
                                <label>세트</label>
                                <input id="세트" placeholder="세트" name="default_set_count" onChange={this.handleChange}/>
                                <label>횟수</label>
                                <input id="횟수" placeholder="횟수" name="default_data" onChange={this.handleChange}/>
                                <label>휴식시간</label>
                                <input id="휴식시간" placeholder="휴식시간" name="default_rest_second" onChange={this.handleChange}/>
                            </div>
                            <div className="input-row">
                                <label className="label-description">영상 링크</label>
                                <input id="link" placeholder="link" name="url" onChange={this.handleChange} onKeyUp={this.handleKeyUp} />
                            </div>
                            
                            <button type="button" onClick={this.AddExercise}>저장하기</button>
                </form>
                <br></br>
                <div className='customerSearch'>
                        ...
                        <input type="text" id='search' name="search" checked={this.state.search} onChange={this.handleChange} />
