...
const postCustomerEnter = (fitness_no, token) => {
  return axios
    .get(`${SERVER_URL}/customerenter`, { params: { fitness_no, token } })
...
class QRLogin extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      member_no: this.props.location.state
        ? Number(this.props.location.state.member_no)
        : 1,
      token: '',
      is_checking: false,
      modal_show: false,
  ...
  getTestCustomerQRCode = () => {
    ...
    fetch(ip + '/customerenter?type=check&type2=customer', {
      method: 'POST',
      headers: {
